...
class TargetTracker:
    def __init__(self):
        ...
        self.kalman = KalmanPredictor(
            process_noise=CFG.kf_process_noise,
            measure_noise=CFG.kf_measure_noise,
        )
    ...
    def _set_locked(self, box):
        ...
        if CFG.predict_enabled:
            # 用卡尔曼滤波平滑检测位置
            fx, fy = self.kalman.update(self.locked_x, self.locked_y)
            self.locked_x = fx
            self.locked_y = fy

    def predicted_position(self):
        """返回预测后的目标位置 (x, y)。
        启用预测 = 卡尔曼位置 + 速度外推 lead_time；
        未启用 = 直接返回当前锁定位置。
        """
        if not CFG.predict_enabled:
            return self.locked_x, self.locked_y
        px, py = self.kalman.predict(CFG.predict_lead_time)
        return px, py
