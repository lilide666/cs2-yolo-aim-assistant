"""
最小延迟视觉伺服 + WASD 周期循环 + 移动稳定后按下

节奏（每 0.25s）：
  周期开始 -> (上轮有目标)按WASD(持续) -> 等0.2s -> YOLO一次
             -> 一步移动 -> 等稳定(MOVE_SETTLE) -> 按下/松开鼠标
             -> 松开WASD -> 补足到0.25s -> 下轮
按 K 暂停/继续
"""
import ctypes
...
TARGET_CLASS = 1
MOVE_K = 2.36
MOVE_SETTLE = 0.05       # ★ 移动后、按下前的稳定延迟(秒)
CLICK_HOLD = 0.02
CLICK_INTERVAL = 0.25
WASD_DELAY = 0.5
K_VK = 0x4B
...
def main():
    ...
    print("启动。周期循环：按WASD→等0.2s→YOLO+移动→等稳定→按下。按K暂停/继续。")
    ...
    try:
        while True:
            ...
            if kk and not k_last:
                paused=not paused
                if paused: release_wasd(); print("已暂停")
                else: print("已继续")
            k_last=kk
            if paused:
            ...
            if last_had_target: press_wasd()
            else: release_wasd()

            time.sleep(WASD_DELAY)
        ...
        pass
    release_wasd(); mouse_up(); cam.release()
