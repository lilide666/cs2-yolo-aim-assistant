...
class Config:
    ...
    # ===== 模型 =====
    model_path: str = r"D:\PycharmProjects\computer_controller\runs\18_model_aug\weights\last.engine"
...
# ===== 控制台配置菜单 =====
#
# 顶层：  ←→ 选择面板(PID/类别/置信度/退出)    ↓ 进入    ESC 暂停/继续
# 每个面板内，选项末尾都有"返回"（回到顶层重新选择，不是退出程序）：
#   PID面板：    ←→ 切 kp/ki/kd/返回       ↑↓ 调大小(返回处=回到顶层)
#   类表面板：    ←→ 移动 0..3/返回          ↑↓ 开/关(返回处=回到顶层)
#   置信度面板：  ←→ 切 置信度/返回          ↑↓ 调阈值(返回处=回到顶层)
#   退出面板(顶层)： ↓ 确认退出程序
#
class ControlMenu:
    ...
    def __init__(self, pid_x, pid_y, class_filter, conf=0.25):
        ...
        self.layer = "top"      # top / pid / cls / conf / exit
        self.panel = "pid"      # top 层游标指向面板
        self.pid_item = "kp"    # PID 面板当前项（含 "back"）
        self.pid_step = 0.01
        self.cls_cursor = 0     # 类表面板光标（0..N，N=类别数；N 即"返回"）
        self.conf_item = "conf" # 置信度面板当前项：conf / back
...
def main():
    ...
    CFG.screen_origin = (left, top)   # 用于把局部坐标转成屏幕绝对坐标
    ...
    print("  每个面板内:  ←→ 移动选项(含末尾[退出])")
    print("              ↑↓ 调节/开关, 停在[退出]按 ↑↓ 退出")
    print("  各面板: ESC 返回顶层")
