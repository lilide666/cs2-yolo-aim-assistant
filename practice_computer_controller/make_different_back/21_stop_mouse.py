...
功能：
    ...
    - 检测到 640 范围内有目标时，持续发送 WASD；无目标时释放
    - PID 控制 + 目标速度前馈
...
WASD_REPEAT_INTERVAL = 0.03   # 持续发送间隔（模拟键盘自动重复）
...
def press_wasd():
    """检测到目标时每帧调用：首次同时按下 WASD，之后按间隔持续发送按下。"""
    global _wasd_active, _wasd_last_send

    if not _wasd_active:
        for vk in WASD_KEYS:
            user32.keybd_event(vk, 0, 0, 0)
        _wasd_active = True
        _wasd_last_send = time.time()
        return

    if time.time() - _wasd_last_send >= WASD_REPEAT_INTERVAL:
        for vk in WASD_KEYS:
            user32.keybd_event(vk, 0, 0, 0)
        _wasd_last_send = time.time()
